...
def excluir_agendamento(agendamento_id):
    ...
    if ag.horario.dia_semana == hoje_nome and hoje.hour >= CUTOFF_HOUR:
        flash(f'Você só pode excluir aulas do mesmo dia até as {CUTOFF_HOUR}:00.', 'danger')
        return redirect(url_for('meus_agendamentos', aluno_id=aluno_id))
...
def editar_agendamento(agendamento_id):
    ag = Agendamento.query.get_or_404(agendamento_id)
    aluno = Aluno.query.get_or_404(ag.aluno_id)
    ...
    if ag.horario.dia_semana == hoje_nome and hoje.hour >= CUTOFF_HOUR:
        flash(f'Edição bloqueada: aulas do mesmo dia só podem ser alteradas até as {CUTOFF_HOUR}:00.', 'danger')
        return redirect(url_for('meus_agendamentos', aluno_id=aluno.id))

    if request.method == 'POST':
        novo_horario_id = request.form.get('horario_id', type=int)
        novo = Horario.query.get_or_404(novo_horario_id)

        if novo.dia_semana == hoje_nome and hoje.hour >= CUTOFF_HOUR:
            flash(f'Edição bloqueada para hoje após {CUTOFF_HOUR}:00.', 'danger')
            return redirect(url_for('meus_agendamentos', aluno_id=aluno.id))

        existe = Agendamento.query.filter_by(aluno_id=aluno.id, horario_id=novo.id).first()
        if existe:
            flash('Você já possui um agendamento nesse horário.', 'warning')
            return redirect(url_for('meus_agendamentos', aluno_id=aluno.id))

        ag.horario_id = novo.id
        db.session.commit()
        flash('Agendamento atualizado com sucesso!', 'success')
        return redirect(url_for('meus_agendamentos', aluno_id=aluno.id))

    horarios_opcoes = Horario.query.filter_by(
        dia_semana=ag.horario.dia_semana,
        faixa=ag.horario.faixa
...
@app.route('/gerenciar_horarios', methods=['GET', 'POST'])
@login_required
